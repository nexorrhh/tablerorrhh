...
async function actualizarBadges() {
  try {
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/solicitudes_empleo?estado=eq.pendiente&select=id`,
      { headers: { apikey: SUPABASE_ANON_KEY, Authorization: `Bearer ${SUPABASE_ANON_KEY}` } }
    );
    if (res.ok) {
      const data = await res.json();
      badges = { ...badges, postulantes: data.length };
    }
  } catch (_) {}
}
...
async function cambiarModulo(id) {
  ...
  renderizarNavLateral(SIDEBAR_ITEMS, modActivo, cambiarModulo, badges);
  renderizarHeader(mod, submodActivo, cambiarSubmodulo);
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Cargar badge de solicitudes pendientes en segundo plano

...
export async function renderizarPanel(contenedor, cambiarModulo) {
  ...
  // ── Carga en paralelo ───────────────────────────────────────────────────────
  const [rEmpleados, rFechas, rPreselMeses, rPostMeses, rIngresos, rContrPanel, rLicPanel, rInstPanel, rClasif] = await Promise.allSettled([
  ...
  if (rEmpleados.status === 'fulfilled') {
    ...
    if (kpiCard) {
      ...
      breakdown.innerHTML = `
        ...
        ${sinAsignar > 0 ? `<div><strong class="panel__kpi-bd-num" style="color:#e67e22">${sinAsignar}</strong><span class="panel__kpi-bd-lbl">Sin clasificar</span></div>` : ''}
      `;
      ...
      if (metaEl && typeof Chart !== 'undefined') {
        ...
        const cfgCat = sinAsignar > 0
          ? pieOpts(['Mensuales', 'Quincenales', 'Sin clasificar'], ['#7c3aed', '#d97706', '#e67e22'])
          : pieOpts(['Mensuales', 'Quincenales'], ['#7c3aed', '#d97706']);
        cfgCat.data.datasets[0].data = sinAsignar > 0 ? [mensuales, quincenales, sinAsignar] : [mensuales, quincenales];

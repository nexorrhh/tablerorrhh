...
import { crearHelpersSupabase, crearToast, e, v, fmtFecha, errHtml } from './venc-comun.js';
import { obtenerUsuario } from '../data/usuario-activo.js';

const TABLA = 'arca_proveedores';
const { sbGet, sbInsert, sbInsertRet, sbUpdate, sbDelete } = crearHelpersSupabase(TABLA);

const perfilesHelper = crearHelpersSupabase('perfiles_rrhh');
const { sbGet: sbGetHist, sbInsert: sbInsertHist } = crearHelpersSupabase('arca_proveedores_acuerdos_historial');

function fmtMonto(m) {
  return m != null ? '$' + Number(m).toLocaleString('es-AR') : null;
}

function acuerdoTexto(r) {
  if (r.fecha_acuerdo == null && r.monto_acordado == null) return '<span class="venc__td-muted">Sin acuerdo</span>';
  const partes = [r.fecha_acuerdo ? fmtFecha(r.fecha_acuerdo) : null, fmtMonto(r.monto_acordado)].filter(Boolean);
  return partes.join(' · ');
}
...
export async function renderizarArcaProveedores(contenedor) {
          ...
          ${registros.length === 0
            ...
            : `<div class="venc__tabla-wrap">
                <table class="venc__tabla">
                  <thead><tr>
                    <th>CUIT</th><th>Razón social</th><th>Plazo de pago</th><th>Autorizador</th><th>Acuerdo</th><th>Estado</th><th></th>
                  </tr></thead>
  ...
  function fila(r) {
    return `
      <tr>
        ...
        <td class="venc__td-muted">${acuerdoTexto(r)}</td>
        <td>
  ...
  function historialItemHtml(h) {
    const fecha = h.fecha_acuerdo ? fmtFecha(h.fecha_acuerdo) : 'Sin fecha';
    const monto = fmtMonto(h.monto_acordado) ?? 'Sin monto';
    return `
      <div class="arca-aut__hist-item">
        <span>${e(fecha)}</span>
        <span class="venc__td-bold">${e(monto)}</span>
        <span class="venc__td-muted">${h.modificado_por ? e(h.modificado_por) : 'Desconocido'} · ${new Date(h.modificado_en).toLocaleString('es-AR')}</span>
      </div>`;
  }

  async function abrirModal(reg = null) {
    editandoId = reg?.id ?? null;
    document.getElementById('venc-modal-proveedor')?.remove();

    let historial = [];
    if (reg) {
      try { historial = await sbGetHist(`proveedor_id=eq.${reg.id}&order=modificado_en.desc`); } catch {}
    }
    ...
    modal.innerHTML = `
      <div class="venc__modal" role="dialog" aria-modal="true">
        ...
        <label class="venc__label">Acuerdo comercial (opcional)</label>
        <input class="venc__input" id="vm-fecha-acuerdo" type="date" value="${reg?.fecha_acuerdo ?? ''}">
        <input class="venc__input" id="vm-monto-acordado" type="number" min="0" step="0.01" value="${reg?.monto_acordado ?? ''}" placeholder="Monto acordado" style="margin-top:8px;">
        <p class="venc__ayuda">Si cargás o cambiás la fecha o el monto, queda registrado en el historial de abajo con tu nombre y la fecha del cambio.</p>

        ${historial.length ? `
          <label class="venc__label">Historial de acuerdos</label>
          <div class="arca-aut__historial">${historial.map(historialItemHtml).join('')}</div>
        ` : ''}
  ...
  async function guardar(cerrar) {
    ...
    const fechaAcuerdo  = v('vm-fecha-acuerdo') || null;
    const montoStr      = v('vm-monto-acordado');
    const montoAcordado = montoStr !== '' ? +montoStr : null;

    const regActual = editandoId ? registros.find(r => r.id === editandoId) : null;
    const cambioAcuerdo = fechaAcuerdo !== (regActual?.fecha_acuerdo ?? null) || montoAcordado !== (regActual?.monto_acordado ?? null);

    const btn = document.getElementById('vm-ok');
    if (btn) { btn.disabled = true; btn.textContent = 'Guardando…'; }
    try {
      let cuit = regActual?.cuit;
      let proveedorId = editandoId;
      if (editandoId) {
        await sbUpdate(editandoId, {
          dias_plazo_pago: diasPlazo, autorizador_default_id: autorizadorId,
          fecha_acuerdo: fechaAcuerdo, monto_acordado: montoAcordado,
        });
      } else {
        cuit = v('vm-cuit').replace(/\D/g, '');
        if (!cuit) { toast('Completá el CUIT', 'error'); if (btn) { btn.disabled = false; btn.textContent = 'Agregar'; } return; }
        const body = {
          cuit, razon_social: null, activo: true, dias_plazo_pago: diasPlazo, autorizador_default_id: autorizadorId,
          fecha_acuerdo: fechaAcuerdo, monto_acordado: montoAcordado,
        };
        if (cambioAcuerdo) proveedorId = (await sbInsertRet(body)).id;
        else await sbInsert(body);
      }
      if (cambioAcuerdo && proveedorId) {
        await sbInsertHist({
          proveedor_id: proveedorId,
          cuit,
          razon_social: regActual?.razon_social ?? null,
          fecha_acuerdo: fechaAcuerdo,
          monto_acordado: montoAcordado,
          modificado_por: obtenerUsuario()?.nombre ?? null,
        });
      }
      cerrar();
      await cargar();

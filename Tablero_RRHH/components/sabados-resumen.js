...
export async function renderizarSabadosResumen(contenedor) {
  ...
  try {
    const [fechasRes, personasRes, empRes, mapaClasif] = await Promise.all([
  ...
  personas.forEach(p => {
    ...
    p.tipo         = empMap.get(String(p.legajo)) || 'sin_asignar';
  });
  const tiposConGente = ['mensual', 'quincenal', 'sin_asignar'].filter(t => personas.some(p => p.tipo === t));
        ...
        <div id="sabres-vista-op">
          ...
          <div class="sabres__tabla-wrap">
            <table class="sabres__tabla">
              <thead id="sabres-thead-op"></thead>
        ...
        <div id="sabres-vista-per" hidden>
          <div class="sabres__vista-subheader">
            <div class="sabres__filtros-bar">
              ...
              <div class="plantel__empresa-pills" id="sabres-pills-tipo-per" role="group" aria-label="Filtrar por tipo de puesto">
                <button class="plantel__pill plantel__pill--activo" data-tipo="">
                  Todos&ensp;<span class="plantel__pill-count">${personas.length}</span>
                </button>
                ${tiposConGente.map(t => `
                <button class="plantel__pill" data-tipo="${t}">
                  ${TIPO_LABEL[t]}&ensp;<span class="plantel__pill-count">${personas.filter(p => p.tipo === t).length}</span>
                </button>`).join('')}
              </div>
          ...
          <div class="sabres__tabla-wrap">
            <table class="sabres__tabla">
              <thead id="sabres-thead-per"></thead>
  ...
  const ordenOp  = crearOrdenTabla('fecha', 'desc');
  const ordenPer = crearOrdenTabla('nombre');

  function valorOp(fila, clave) {
    switch (clave) {
      case 'fecha':        return fila.fecha;
      case 'dia':           return fila.dia_semana || diaDe(fila.fecha);
      case 'tipo':          return fila.tipo;
      case 'convocados':    return fila.convocados;
      case 'presentes':     return fila.presentes;
      case 'ausentes':      return fila.ausentes;
      case 'noconv':        return fila.no_convocados;
      case 'cumplimiento':  return fila.pct;
      default:              return null;
    }
  }

  function valorPer(fila, clave) {
    switch (clave) {
      case 'legajo':        return +fila.legajo;
      case 'nombre':        return fila.apellido_y_nombre || '';
      case 'empresa':       return fila.empresa || '';
      case 'conv':          return fila.convocado;
      case 'pres':          return fila.presente;
      case 'aus':           return fila.ausente;
      case 'noconv':        return fila.no_convocado;
      case 'cumplimiento':  return fila.pct;
      default:              return null;
    }
  }

  function theadOpHtml() {
    return `<tr>
      ${ordenOp.thHtml('fecha', 'Fecha')}
      ${ordenOp.thHtml('dia', 'Día')}
      ${ordenOp.thHtml('tipo', 'Tipo')}
      ${ordenOp.thHtml('convocados', 'Convocados', { clase: 'sabres__td--right' })}
      ${ordenOp.thHtml('presentes', 'Presentes', { clase: 'sabres__td--right' })}
      ${ordenOp.thHtml('ausentes', 'Ausentes', { clase: 'sabres__td--right' })}
      ${ordenOp.thHtml('noconv', 'No conv.', { clase: 'sabres__td--right' })}
      ${ordenOp.thHtml('cumplimiento', 'Cumplimiento')}
    </tr>`;
  }

  function theadPerHtml() {
    return `<tr>
      ${ordenPer.thHtml('legajo', 'Legajo')}
      ${ordenPer.thHtml('nombre', 'Apellido y nombre')}
      ${ordenPer.thHtml('empresa', 'Empresa')}
      ${ordenPer.thHtml('conv', 'Conv.', { clase: 'sabres__td--right' })}
      ${ordenPer.thHtml('pres', 'Pres.', { clase: 'sabres__td--right' })}
      ${ordenPer.thHtml('aus', 'Aus.', { clase: 'sabres__td--right' })}
      ${ordenPer.thHtml('noconv', 'No conv.', { clase: 'sabres__td--right' })}
      ${ordenPer.thHtml('cumplimiento', 'Cumplimiento')}
    </tr>`;
  }
  ...
  function actualizarTablaPersonas() {
    ...
    if (filtroTipoPer)    lista = lista.filter(p => p.tipo === filtroTipoPer);
    if (filtroTextoPer) {
      const txt = filtroTextoPer.toLowerCase();
      lista = lista.filter(p => (p.apellido_y_nombre || '').toLowerCase().includes(txt));
    }
    lista = lista.sort(ordenPer.comparador(valorPer));
    const visibles = expandedPer ? lista : lista.slice(0, LIMITE);
    contenedor.querySelector('#sabres-thead-per').innerHTML = theadPerHtml();
  ...
  // ── Tipo de puesto (persona) ───────────────────────────────────────────────
  contenedor.querySelectorAll('#sabres-pills-tipo-per [data-tipo]').forEach(btn => {
    btn.addEventListener('click', () => {
      contenedor.querySelectorAll('#sabres-pills-tipo-per [data-tipo]')
        .forEach(b => b.classList.remove('plantel__pill--activo'));
      btn.classList.add('plantel__pill--activo');
      filtroTipoPer = btn.dataset.tipo;
      expandedPer = false;
      actualizarTablaPersonas();
    });
  });
  ...
  ordenOp.wire(contenedor.querySelector('#sabres-vista-op'), () => { expandedOp = false; actualizarTablaOp(); });
  ordenPer.wire(contenedor.querySelector('#sabres-vista-per'), () => { expandedPer = false; actualizarTablaPersonas(); });
  ...
  // ── Gráficos ────────────────────────────────────────────────────────────────
  const g1 = crearLineaEvo(contenedor, fechas);
  const g2 = crearDonaAsistencia(contenedor, totPres, totAus, totNoc);
...
function crearLineaEvo(contenedor, fechas) {
  const canvas = contenedor.querySelector('#sabres-evo');
...
function crearDonaAsistencia(contenedor, presentes, ausentes, noConvocados) {
  const canvas = contenedor.querySelector('#sabres-dona');
  if (!canvas || typeof Chart === 'undefined') return null;
  return new Chart(canvas.getContext('2d'), {
    type: 'doughnut',
    data: {

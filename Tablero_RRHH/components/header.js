...
import { modoActual, alternarModo } from '../data/tema-modo.js';
...
export function renderizarHeader(mod, submodActivoId, alCambiarSubmod, opciones = {}) {
  ...
  const oscuro = modoActual() === 'oscuro';
    ...
    <div class="page-header">
      <div class="page-header__top">
        ...
        <div class="page-header__usuario">
          <button class="page-header__btn-tema" id="btn-tema" type="button"
                  aria-pressed="${oscuro}" title="${oscuro ? 'Cambiar a modo claro' : 'Cambiar a modo noche'}">
            ${oscuro ? '☀️' : '🌙'}
          </button>
          ${usuario ? `
  ...
  header.querySelector('#btn-tema')?.addEventListener('click', e => {
    const oscuroAhora = alternarModo() === 'oscuro';
    const btn = e.currentTarget;
    btn.textContent = oscuroAhora ? '☀️' : '🌙';
    btn.setAttribute('aria-pressed', String(oscuroAhora));
    btn.title = oscuroAhora ? 'Cambiar a modo claro' : 'Cambiar a modo noche';
  });
}

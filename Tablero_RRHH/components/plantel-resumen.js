...
export async function renderizarPlantelResumen(contenedor) {
  ...
  let empleados, mapaClasif;
  try {
    [empleados, mapaClasif] = await Promise.all([
  ...
  const mensuales   = empleados.filter(e => tipoPuesto(e.desc_puesto, mapaClasif) === 'mensual');
  const quincenales = empleados.filter(e => tipoPuesto(e.desc_puesto, mapaClasif) === 'quincenal');
  const sinAsignar  = empleados.filter(e => tipoPuesto(e.desc_puesto, mapaClasif) === 'sin_asignar');
  ...
  const sinCim   = sinAsignar.filter(e => e.empresa === 'CIMOMET').length;
  const sinCom   = sinAsignar.filter(e => e.empresa === 'COMOING').length;

  const mapMensual   = agruparPorPuesto(mensuales);
  const mapQuincenal = agruparPorPuesto(quincenales);
  const mapSinAsignar = agruparPorPuesto(sinAsignar);
  ...
  const maxSin   = Math.max(...Object.values(mapSinAsignar).map(v => v.total), 1);
  const haySinAsignar = sinAsignar.length > 0;
  ...
  contenedor.innerHTML = `
    <div class="plantel">
      ...
      <div class="plantel__stats plantel__stats--5">
        ...
        ${haySinAsignar ? `
        <div class="plantel__stat-card" style="border-top-color:${COLOR_SIN_ASIGNAR}">
          <p class="plantel__stat-valor" style="color:${COLOR_SIN_ASIGNAR}">${sinAsignar.length}</p>
          <p class="plantel__stat-label">Sin clasificar</p>
        </div>` : ''}
      </div>
      ...
      <div class="plantel__graficos">
        ...
        <div class="plantel__grafico-card">
          ...
          <div class="plantel__grafico-leyenda">
            ...
            ${haySinAsignar ? filaLeyenda(COLOR_SIN_ASIGNAR, 'Sin clasificar', sinAsignar.length, total) : ''}
          </div>
        </div>
          ...
          <table class="plantel__cruce-tabla">
            <thead>
              <tr>
                ...
                ${haySinAsignar ? '<th><span class="plantel__badge plantel__badge--sinasignar">Sin clasificar</span></th>' : ''}
            ...
            <tbody>
              ...
              <tr class="plantel__cruce-total">
                ...
                ${haySinAsignar ? `<td class="plantel__cruce-bold">${sinAsignar.length}</td>` : ''}
      ...
      ${haySinAsignar ? renderSeccion('Sin clasificar', COLOR_SIN_ASIGNAR, mapSinAsignar, maxSin, sinAsignar.length, sinCim, sinCom) : ''}

    </div>
  `;

  // Instanciar gráficos Chart.js una vez que el DOM está listo
  const g1 = crearDonut(contenedor, 'grafico-empresas',
    [cimomet, comoing], ['Cimomet', 'Co.mo.ing'],
    [COLOR_CIMOMET, COLOR_COMOING]);
  const g2 = crearDonut(contenedor, 'grafico-sectores',
    [mensuales.length, quincenales.length, ...(haySinAsignar ? [sinAsignar.length] : [])],
    ['Mensuales', 'Quincenales', ...(haySinAsignar ? ['Sin clasificar'] : [])],
    [COLOR_MENSUAL, COLOR_QUINCENAL, ...(haySinAsignar ? [COLOR_SIN_ASIGNAR] : [])]);
...
function crearDonut(contenedor, canvasId, datos, etiquetas, colores) {
  const canvas = contenedor.querySelector('#' + canvasId);

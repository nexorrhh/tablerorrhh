...
// "Apellido, Nombre Segundo" → "Nombre" (primer nombre de pila), para un saludo natural.
// Si no viene en ese formato, usa la primera palabra de lo que haya.
function primerNombreDe(nombreCompleto) {
  const partes = nombreCompleto.split(',');
  const nombrePila = (partes[1] || partes[0] || '').trim();
  return nombrePila.split(/\s+/)[0] || nombreCompleto;
}

// Saludo efímero tras un login exitoso — aparece, se queda un instante y desaparece solo,
// dejando entrar al tablero. `onResolve` es lo que ya hacía avanzar el login (no cambia el
// contrato de mostrarLogin(), solo se demora un toque para mostrar el saludo primero).
function mostrarSaludo(overlay, nombre, usuario, onResolve) {
  overlay.innerHTML = `
    <div class="login__saludo" id="login-saludo">
      <span class="login__saludo-emoji" aria-hidden="true">👋</span>
      <h2 class="login__saludo-texto">¡Hola, ${e(primerNombreDe(nombre))}!</h2>
    </div>`;
  const saludo = overlay.querySelector('#login-saludo');
  requestAnimationFrame(() => requestAnimationFrame(() => saludo.classList.add('login__saludo--entrar')));

  setTimeout(() => {
    saludo.classList.add('login__saludo--salir');
    setTimeout(() => {
      overlay.remove();
      onResolve(usuario);
    }, 300);
...
function permisosDe(perfil) {
  ...
  if (perfil?.puede_autorizar_busquedas) permisos.push('autorizarBusquedas');
  if (perfil?.puede_eliminar_operativos) permisos.push('eliminarOperativos');
  return permisos;
}
...
// ── Transición entre pantallas del login (fade + leve movimiento, mismo "aire" que el
// saludo de bienvenida) ───────────────────────────────────────────────────────
const DURACION_SALIDA = 170; // ms — tiene que coincidir con .login__card--saliendo

// Reemplaza el contenido del overlay por una pantalla nueva sin el corte seco de un
// innerHTML directo: la tarjeta actual se desvanece, y la nueva entra con el mismo
// rebote suave que el saludo. `renderPantalla` es la función que arma la pantalla nueva
// (ya hace su propio overlay.innerHTML = ...), se le pasa tal cual, solo se demora un toque.
function fadeSwap(overlay, renderPantalla) {
  const actual = overlay.querySelector('.login__card');
  if (!actual) { renderPantalla(); animarEntrada(overlay); return; }
  actual.classList.add('login__card--saliendo');
  setTimeout(() => {
    renderPantalla();
    animarEntrada(overlay);
  }, DURACION_SALIDA);
}

function animarEntrada(overlay) {
  const nueva = overlay.querySelector('.login__card');
  if (!nueva) return;
  nueva.classList.add('login__card--pre-entrada');
  requestAnimationFrame(() => requestAnimationFrame(() => {
    nueva.classList.remove('login__card--pre-entrada');
    nueva.classList.add('login__card--entrando');
  }));
}
...
function renderSeleccion(overlay, perfiles, onResolve) {
  ...
  overlay.querySelectorAll('.login__perfil[data-id]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (tienePin) {
        fadeSwap(overlay, () => renderIngresarPin(overlay, perfiles, id, nombre, onResolve));
      } else {
        fadeSwap(overlay, () => renderCrearPin(overlay, perfiles, id, nombre, onResolve));
      }
    });
  });
}
...
function renderIngresarPin(overlay, perfiles, id, nombre, onResolve) {
  ...
  overlay.querySelector('#login-volver').addEventListener('click', () => {
    fadeSwap(overlay, () => renderSeleccion(overlay, perfiles, onResolve));
  });
  ...
  async function intentarLogin() {
    ...
    if (perfil && perfil.password_hash === hash) {
      const usuario = { id, nombre, permisos: permisosDe(perfil), modulosPermitidos: perfil.modulos_permitidos || null };
      guardarSesion(usuario);
      mostrarSaludo(overlay, nombre, usuario, onResolve);
    } else {
      error.hidden = false;
      input.value = '';
      input.focus();
...
export function mostrarLogin() {
  return new Promise(async (resolve) => {
    ...
    // Fundido del fondo del overlay — se nota sobre todo al cerrar sesión, cuando aparece
    // por encima del tablero (en el primer ingreso la página está en blanco atrás igual).
    requestAnimationFrame(() => requestAnimationFrame(() => overlay.classList.add('login__overlay--visible')));

...
function tablaDetalleAusentismo(rawGroup, periodo, filtroEmpresa, activosSet) {
  const filas = rawGroup
    .filter(d => d.periodo === periodo && EMPRESAS.includes(d.empresa) && (!filtroEmpresa || d.empresa === filtroEmpresa) && (+d.hs_esperadas || 0) > 0)
    .map(d => {
...
function datosAusentismoExport(rawGroup, activosSet, periodos) {
  const porEmpleado = [];
  const porSectorPeriodo = new Map(); // `periodo|sector` → acumulado

  periodos.forEach(periodo => {
    rawGroup
      .filter(d => d.periodo === periodo && EMPRESAS.includes(d.empresa) && (+d.hs_esperadas || 0) > 0)
      .forEach(d => {
        const total = +d.hs_ausencias || 0;
        if (Math.round(total) <= 0) return;
        const just   = Math.min(+d.hs_justificadas || 0, total);
        const nojust = total - just;
        const sector = d.departamento || '—';
        const nombre = `${d.apellido || ''}, ${d.nombre || ''}`.trim().replace(/^,\s*/, '');
        const activo = !activosSet || activosSet.has(String(d.legajo));

        porEmpleado.push([
          fmtPeriodo(periodo), sector, d.legajo, nombre, EMP_LABEL[d.empresa] || d.empresa,
          activo ? 'Sí' : 'No',
          Math.round(just * 100) / 100, Math.round(nojust * 100) / 100, Math.round(total * 100) / 100,
        ]);

        const key = `${periodo}|${sector}`;
        if (!porSectorPeriodo.has(key)) porSectorPeriodo.set(key, { periodo, sector, just: 0, nojust: 0, count: 0 });
        const s = porSectorPeriodo.get(key);
        s.just += just; s.nojust += nojust; s.count++;
      });
  });

  porEmpleado.sort((a, b) => a[0].localeCompare(b[0]) || a[1].localeCompare(b[1]) || b[8] - a[8]);

  const resumen = [...porSectorPeriodo.values()]
    .sort((a, b) => a.periodo.localeCompare(b.periodo) || (b.just + b.nojust) - (a.just + a.nojust))
    .map(s => [
      fmtPeriodo(s.periodo), s.sector, s.count,
      Math.round(s.just * 100) / 100, Math.round(s.nojust * 100) / 100, Math.round((s.just + s.nojust) * 100) / 100,
    ]);

  return { porEmpleado, resumen };
}

function exportarAusentismoExcel(rawGroup, activosSet, periodos, nombreArchivo) {
  if (typeof XLSX === 'undefined') {
    alert('La librería de Excel no está disponible. Verificá la conexión a internet.');
    return;
  }
  const { porEmpleado, resumen } = datosAusentismoExport(rawGroup, activosSet, periodos);
  const wb = XLSX.utils.book_new();

  const ws1 = XLSX.utils.aoa_to_sheet([
    ['Período', 'Sector', 'Legajo', 'Nombre', 'Empresa', 'Activo', 'Hs. Justificadas', 'Hs. No justificadas', 'Total'],
    ...porEmpleado,
  ]);
  ws1['!cols'] = [12, 22, 10, 28, 12, 8, 16, 18, 10].map(w => ({ wch: w }));
  XLSX.utils.book_append_sheet(wb, ws1, 'Detalle por empleado');

  const ws2 = XLSX.utils.aoa_to_sheet([
    ['Período', 'Sector', 'Empleados', 'Hs. Justificadas', 'Hs. No justificadas', 'Total'],
    ...resumen,
  ]);
  ws2['!cols'] = [12, 22, 12, 16, 18, 10].map(w => ({ wch: w }));
  XLSX.utils.book_append_sheet(wb, ws2, 'Resumen por sector');
...
function renderDias(dias) {
  // Solo días de falta completa: tenía jornada programada y no registró ninguna hora real
  const conAus = dias.filter(d => (+d.hs_reales || 0) < 0.005 && (+d.hs_esperadas || 0) > 0.005);
...
export async function renderizarPresentismoIndicadores(contenedor) {
  ...
  let rawHoras = [], empleados = [], rawAusTipo = [], detalleExt50 = [], rawTardanzas = [], mapaClasif = new Map();
  try {
    const [rH, rE, rA, rDrows, rTard, mapa] = await Promise.all([
      ...
      fetchTodasFilas(`${SUPABASE_URL}/rest/v1/rrhh_horas_detalle?tipo_hora=in.(HSEXT,HSEXT50)&select=legajo,periodo,fecha,hs_trabajadas`),
      fetchTodasFilas(`${SUPABASE_URL}/rest/v1/rrhh_tardanzas_salidas?select=legajo,periodo,fecha,tipo,minutos,descripcion_justificacion`),
  ...
  const rawTardQ = rawTardanzas.filter(t => empMap.get(String(t.legajo))?.tipo === 'quincenal');
  const rawTardM = rawTardanzas.filter(t => empMap.get(String(t.legajo))?.tipo === 'mensual');
  ...
  ['q', 'm'].forEach(sufijo => {
    ...
    btnExport?.addEventListener('click', () => {
      ...
      exportarAusentismoExcel(rawGroup, activosSet, periodos, `Ausentismo_${nombreGrupo}_${sufijoArchivo}.xlsx`);
    });
    ...
    tablaDiv?.addEventListener('click', async e => {
      ...
      if (abriendo && !expandRow.dataset.loaded) {
        ...
        try {
          const r = await fetch(`${SUPABASE_URL}/rest/v1/rrhh_horas_detalle?legajo=eq.${legajo}&periodo=eq.${per}&tipo_hora=eq.HSNOR&select=fecha,descripcion_tipo_hora,hs_reales,hs_esperadas,hs_justificadas,hs_no_justificadas&order=fecha.asc`, { headers: HDR });

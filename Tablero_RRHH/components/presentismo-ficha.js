    ...
    if (!datos) {
      try {
        ...
        const otRows = persona?.empresa
          ? await fetchTodasFilas(`${SUPABASE_URL}/rest/v1/horas_ot_detalle?legajo=eq.${legajo}&empresa=eq.${persona.empresa}&anomalo=eq.false&select=fecha,periodo,ot,operacion,horas&order=id.asc`)
          : [];

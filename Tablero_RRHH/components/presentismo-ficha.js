  ...
  function renderFicha(legajo, { rows, eventos, detalle, otRows, persona }) {
    ...
    contFicha.innerHTML = `
      <div class="pind__sec pind__sec--grupo">
        ...
        <div class="pind__graf-grid" style="margin-top:var(--espacio-m)">
          <div class="pind__sec pind__graf-card" style="background:var(--color-fondo);grid-column:1 / -1">
            <h3 class="pind__sec-tit">Horas por OT</h3>
            <div id="ficha-ot-wrap" class="pind__graf-wrap"><canvas id="ficha-ch-ot"></canvas></div>
          </div>
        </div>
    ...
    // Horas por OT y período — se agrupan en el top 8 por horas totales (el resto cae en
    // "Otras OT") para que la leyenda no quede ilegible en alguien que pasó por muchas órdenes.
    const otValidas = otRows.filter(o => !o.anomalo);
    const otTotales = new Map();
    otValidas.forEach(o => {
      const clave = o.ot || 'Sin OT';
      otTotales.set(clave, (otTotales.get(clave) || 0) + (+o.horas || 0));
    });
    const TOP_OT = 8;
    const otsOrdenadas = [...otTotales.entries()].sort((a, b) => b[1] - a[1]);
    const otsTop = new Set(otsOrdenadas.slice(0, TOP_OT).map(([ot]) => ot));
    const hayOtrasOt = otsOrdenadas.length > TOP_OT;
    const otPorPeriodo = new Map();
    [...otsTop, ...(hayOtrasOt ? ['Otras OT'] : [])].forEach(ot => otPorPeriodo.set(ot, allPeriodos.map(() => 0)));
    otValidas.forEach(o => {
      const clave = o.ot || 'Sin OT';
      const destino = otsTop.has(clave) ? clave : (hayOtrasOt ? 'Otras OT' : clave);
      const idx = allPeriodos.indexOf(o.periodo);
      if (idx >= 0 && otPorPeriodo.has(destino)) otPorPeriodo.get(destino)[idx] += (+o.horas || 0);
    });

    if (!otValidas.length) {
      contFicha.querySelector('#ficha-ot-wrap').innerHTML = '<p style="padding:24px 0;text-align:center;color:var(--color-texto-sec);font-size:0.85rem">Sin horas por OT cargadas para esta persona.</p>';
    } else {
      const PALETA_OT = ['#1e3a5f', '#2563eb', '#16a34a', '#d97706', '#dc2626', '#7c3aed', '#0891b2', '#be185d'];
      const clavesOt = [...otPorPeriodo.keys()];
      graficosFicha.push(new Chart(contFicha.querySelector('#ficha-ch-ot').getContext('2d'), {
        type: 'bar',
        data: { labels, datasets: clavesOt.map((ot, i) => ({
          label: ot, data: otPorPeriodo.get(ot).map(v => Math.round(v)),
          backgroundColor: ot === 'Otras OT' ? '#94a3b8' : PALETA_OT[i % PALETA_OT.length],
          borderRadius: 4, stack: 'ot',
        })) },
        options: { responsive: true, maintainAspectRatio: false,
          plugins: { legend: { position: 'top', labels: { boxWidth: 12 } },
            tooltip: { callbacks: { label: c => ` ${c.dataset.label}: ${c.parsed.y.toLocaleString('es-AR')}h` } } },

...
function parsearExcel(buffer) {
  ...
  for (let i = 1; i < filas.length; i++) {
    ...
    const diaKey = fecha;
    if (!agg.diasTracked.has(diaKey)) agg.diasTracked.set(diaKey, { reales: 0, justificadas: 0, esLaborable: false });
  ...
  for (const [, agg] of byLegajo) {
    ...
    let dias_laborables = 0, dias_presentes = 0, dias_ausentes_nojust = 0;
    for (const [, dia] of agg.diasTracked) {
      if (!dia.esLaborable) continue; // solo días con fila HSNOR son laborables
      dias_laborables++;
      if (dia.reales > 0) {
        dias_presentes++;
      } else if (dia.justificadas === 0) {
        dias_ausentes_nojust++; // ni trabajó ni justificó → falta
      }
      // justificadas > 0 y trabajadas = 0 → licencia/vacación, no cuenta ni para ni contra
    }
...
async function guardarEnSupabase(mensual, detalle, periodo) {
  ...
  // Solo si el mensual fue ok, reemplazar el detalle
  await fetch(`${SUPABASE_URL}/rest/v1/rrhh_horas_detalle?periodo=eq.${periodo}`, {

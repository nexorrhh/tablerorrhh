...
// Parsea el Excel y devuelve { mensual, detalle, tardanzas, periodo }
function parsearExcel(buffer) {
  ...
  // parsean ANTES del loop principal porque las ausencias por vacación necesitan
  // reclasificar filas de "Detalle de horas" del mismo archivo (ver más abajo).
  const eventos = parsearTardanzas(wb);
  // Tango a veces deja estos días con 0h esperadas y todo el déficit como "no justificado"
  // en "Detalle de horas", aunque "Horas no trabajadas" los marca como VACACION reales.
  ...
  for (let i = 1; i < filas.length; i++) {
    ...
    // Vacación no trabajada mal reflejada en esta hoja (0h esperadas, todo el déficit como
    // no justificado) — se reclasifica usando el motivo real de "Horas no trabajadas".
    if (tipoHora === 'HSNOR' && espDiur + espNoct === 0 && (noJustDiur + noJustNoct) > 0
        && vacacionPorDia.has(`${legajo}|${fecha}`)) {
      espDiur = noJustDiur;  espNoct = noJustNoct;
      justDiur = noJustDiur; justNoct = noJustNoct;
    ...
    if (tipoHora === 'HSNOR') {
    ...
    } else if (tipoHora === 'HSEXT' || tipoHora === 'HSEXT50') {
      ...
      // si algún export previo o futuro usara ese código explícito.
  ...
  contenedor.innerHTML = `
    <div class="pres__carga-wrap">
      <div class="pres__carga-cabecera">
        ...
        <p class="pres__subtitulo">Seleccioná el archivo <strong>"Extenso"</strong> exportado desde Tango en formato <strong>.xlsx</strong> (también acepta el reporte simple "Detalle de horas", pero sin tardanzas ni salidas anticipadas).</p>
      </div>

...
// Normaliza un encabezado de columna para poder reconocerlo pase lo que pase con el formato
// exacto del export de Tango: a veces usa "Número de legajo" (con espacios y tildes) y a
// veces "Numero_de_legajo" (con guion bajo y sin tildes) según el tipo de reporte. Sacando
// tildes, guiones bajos y espacios repetidos, ambas variantes matchean igual.
const DIACRITICOS_NFD = new RegExp('[̀-ͯ]', 'g');
function normalizarHeader(s) {
  return String(s).trim().toLowerCase()
    .normalize('NFD').replace(DIACRITICOS_NFD, '') // saca tildes
    .replace(/[_.]+/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
}
...
function parsearTardanzas(wb) {
  ...
  const col = name => headers.findIndex(h => h === normalizarHeader(name));
  const C = {
...
// Lee la empresa DIRECTAMENTE del archivo (hoja "Datos de la Empresa" → "Nombre legal"),
// en vez de adivinarla cruzando el legajo contra la tabla de empleados. Es la fuente correcta:
// Cimomet y Co.mo.ing pueden tener el MISMO número de legajo para dos personas distintas (cada
// una es una empresa con su propia numeración en Tango), así que "a qué empresa pertenece este
// legajo" no se puede resolver de forma confiable mirando solo el número — hay que preguntarle
// al archivo mismo, que es un export de una sola empresa por vez.
function empresaDelArchivo(wb) {
  const ws = wb.Sheets['Datos de la Empresa'];
  if (!ws) return null;
  const filas = XLSX.utils.sheet_to_json(ws, { header: 1, defval: '' });
  // El export nuevo de Tango duplica la fila de encabezado (una versión snake_case sin
  // tildes y otra con espacios/tildes) antes de la fila de datos — normalizarHeader hace que
  // las dos matcheen "nombre legal", así que hay que quedarse con la ÚLTIMA fila de encabezado
  // que matchea, no la primera, para no leer otro encabezado en vez del dato real.
  let idxHeaderRow = -1;
  for (let i = 0; i < filas.length; i++) {
    if (filas[i].some(c => normalizarHeader(c) === 'nombre legal')) idxHeaderRow = i;
    else if (idxHeaderRow !== -1) break; // ya pasamos el bloque de encabezados
  }
  if (idxHeaderRow === -1) return null;
  const idxCol = filas[idxHeaderRow].findIndex(c => normalizarHeader(c) === 'nombre legal');
  const nombreLegal = String(filas[idxHeaderRow + 1]?.[idxCol] || '').toUpperCase();
  if (nombreLegal.includes('CIMOMET')) return 'CIMOMET';
  if (nombreLegal.replace(/[.\s]/g, '').includes('COMOING')) return 'COMOING';
  return null;
}

// Parsea el Excel de Tango y devuelve { mensual, detalle, tardanzas, periodo, empresa }.
// Recibe el workbook ya leído (ver detectarTipoArchivo) para no leer el archivo dos veces.
function parsearExcel(wb) {
  const empresaArchivo = empresaDelArchivo(wb);
  ...
  const headers = filas[0].map(normalizarHeader);
  const col = name => headers.findIndex(h => h === normalizarHeader(name));
  // Algunos exports nuevos de Tango duplican cada columna de horas: una "Horas X diurna"
  // (sin "Hs.") con el valor real EN MINUTOS, y una "Hs. X diurna" que queda vacía y que en
  // los exports viejos sí trae el valor pero ya convertido a horas decimales. colConFactor
  // junta, para cada nombre posible, su índice de columna y el factor para llevarlo a horas
  // (1/60 si es la variante en minutos, 1 si ya viene en horas). A la hora de leer cada fila,
  // valorCol usa la primera variante que tenga valor no vacío — así funciona con cualquiera
  // de los dos formatos sin tener que adivinar cuál es "el bueno" de antemano.
  const colConFactor = (...pares) => pares
    .map(([nombre, factor]) => ({ idx: col(nombre), factor }))
    .filter(c => c.idx !== -1);
  const colTodas = (...nombresEnHoras) => colConFactor(...nombresEnHoras.map(n => [n, 1]));
  // Para columnas sin variantes (nunca duplicadas) — nombre exacto, error si falta.
  ...
  const valorCol = (f, cols) => {
    for (const { idx, factor } of cols) {
      const v = f[idx];
      if (v !== '' && v !== null && v !== undefined) return (+v || 0) * factor;
    }
    return 0;
  };
  ...
  const C = {
    ...
    espDiur:     colConFactor(['Horas esperadas diurna', 1 / 60], ['Hs. esperadas diurna', 1]),
    espNoct:     colConFactor(['Horas esperadas nocturna', 1 / 60], ['Hs. esperadas nocturna', 1]),
    trabDiur:    colConFactor(['Horas trabajadas diurna', 1 / 60], ['Hs. trabajadas diurna', 1]),
    trabNoct:    colConFactor(['Horas trabajadas nocturna', 1 / 60], ['Hs. trabajadas nocturna', 1]),
    justDiur:    colConFactor(['Horas no trab justif diurna', 1 / 60], ['Hs. no trabajadas justificadas diurna', 1], ['Hs. no trab justif diurna', 1]),
    justNoct:    colConFactor(['Horas no trab justif nocturna', 1 / 60], ['Hs. no trabajadas justificadas nocturna', 1], ['Hs. no trab justif nocturna', 1]),
    noJustDiur:  colConFactor(['Horas no trab no justif diurna', 1 / 60], ['Hs. no trabajadas no justificadas diurna', 1], ['Hs. no trab no justif diurna', 1]),
    noJustNoct:  colConFactor(['Horas no trab no justif noctur', 1 / 60], ['Hs. no trabajadas no justificadas nocturna', 1], ['Hs. no trab no justif noctur', 1]),
    realDiur:    colConFactor(['Horas reales diurna', 1 / 60], ['Hs. reales diurna', 1]),
    realNoct:    colConFactor(['Horas reales nocturna', 1 / 60], ['Hs. reales nocturna', 1]),
    descTipo:    col('Descripción de tipo hora'), // opcional — para mostrar motivo de justificación
  };

  // descTipo es opcional; las columnas de horas son válidas si tienen AL MENOS una variante; el resto son requeridas
  const COLUMNAS_HORAS = new Set(['espDiur', 'espNoct', 'trabDiur', 'trabNoct', 'justDiur', 'justNoct', 'noJustDiur', 'noJustNoct', 'realDiur', 'realNoct']);
  const faltantes = Object.entries(C)
    .filter(([k, v]) => k !== 'descTipo' && (COLUMNAS_HORAS.has(k) ? v.length === 0 : v === -1))
    .map(([k]) => k);
  ...
  for (let i = 1; i < filas.length; i++) {
    ...
    let   trabDiur   = +valorCol(f, C.trabDiur)   || 0;
    let   trabNoct   = +valorCol(f, C.trabNoct)   || 0;
    let   justDiur   = +valorCol(f, C.justDiur)   || 0;
    let   justNoct   = +valorCol(f, C.justNoct)   || 0;
    let   noJustDiur = +valorCol(f, C.noJustDiur) || 0;
    let   noJustNoct = +valorCol(f, C.noJustNoct) || 0;
    let   espDiur    = +valorCol(f, C.espDiur)    || 0;
    let   espNoct    = +valorCol(f, C.espNoct)    || 0;
    let   realDiur   = +valorCol(f, C.realDiur)   || 0;
    let   realNoct   = +valorCol(f, C.realNoct)   || 0;
    ...
    detalleRows.push({
      legajo, fecha, tipo_hora: tipoHora, empresa: empresaArchivo,
  ...
  // Sin saber a qué empresa pertenece el archivo no se puede guardar nada con seguridad: Cimomet
  // y Co.mo.ing pueden compartir el mismo número de legajo para personas distintas, así que
  // guardar "a ciegas" arriesga pisar los datos de otra persona.
  if (!empresaArchivo) {
    throw new Error(
      'No pude determinar la empresa de este archivo (hoja "Datos de la Empresa" no encontrada o ' +
      'con un nombre legal irreconocible). No se guarda nada hasta confirmar la empresa.'
    );
  }

  // Período = primer día del mes de la fecha mínima
  const minSerial = Math.min(...fechasSeriales);
  const maxSerial = Math.max(...fechasSeriales);
  const minFecha  = new Date(Date.UTC(1899, 11, 30) + minSerial * 86400000);
  const maxFecha  = new Date(Date.UTC(1899, 11, 30) + maxSerial * 86400000);
  const periodo   = `${minFecha.getUTCFullYear()}-${String(minFecha.getUTCMonth() + 1).padStart(2, '0')}-01`;
  const periodoMax = `${maxFecha.getUTCFullYear()}-${String(maxFecha.getUTCMonth() + 1).padStart(2, '0')}-01`;

  // Cada carga es UN período — si el archivo trae fechas de más de un mes calendario, TODAS
  // esas filas se etiquetarían igual con el período de la fecha más vieja (mezclando meses
  // distintos bajo un solo período, y de paso pisando por upsert los datos reales que ya
  // hubiera cargados para esos otros meses). Mejor cortar acá con un aviso claro que dejar
  // pasar un archivo que no es el reporte mensual esperado.
  if (periodo !== periodoMax) {
    throw new Error(
      `Este archivo tiene fechas de varios meses (de ${formatearPeriodo(periodo)} a ${formatearPeriodo(periodoMax)}). ` +
      `El informe de Tango se debe generar para un solo mes, desde el primer día hasta el último día de ese mes — ` +
      `no un acumulado de varios meses juntos. Volvé a generarlo en Tango con ese rango y subilo de nuevo.`
    );
  }
  ...
  for (const [, agg] of byLegajo) {
    ...
    for (const [, dia] of agg.diasTracked) {
      ...
      if (dia.reales > 0) {
        ...
        // reales = 0 con jornada programada → falta completa. Cuenta para "días presente" esté
        // o no justificada: enfermedad, accidente, etc. igual es un día que no vino a trabajar,
        // aunque no se lo vaya a sancionar por eso (eso es lo que indica la justificación).
  ...
  const tardanzas = eventos.map(t => ({ ...t, periodo, empresa: empresaArchivo }));

  return { mensual, detalle: detalleRows, tardanzas, periodo, empresa: empresaArchivo };
}

async function guardarEnSupabase(mensual, detalle, tardanzas, periodo, empresa) {
  // UPSERT del resumen mensual: nunca borra datos existentes antes de confirmar
  // Si falla, los datos anteriores quedan intactos.
  // La clave incluye empresa: Cimomet y Co.mo.ing pueden tener el mismo número de legajo
  // para personas distintas, así que "período + legajo" solo no alcanza para identificar la fila.
  const rMens = await fetch(`${SUPABASE_URL}/rest/v1/rrhh_horas_mensual?on_conflict=periodo,legajo,empresa`, {
    method: 'POST',
  ...
  // Solo si el mensual fue ok, reemplazar el detalle de los legajos de este archivo — acotado
  // también por empresa, por la misma razón: no pisar el detalle de un legajo igual de la otra empresa.
  const legajosArchivo = [...new Set(detalle.map(d => d.legajo))];
  await fetch(`${SUPABASE_URL}/rest/v1/rrhh_horas_detalle?periodo=eq.${periodo}&empresa=eq.${empresa}&legajo=in.(${legajosArchivo.join(',')})`, {
  ...
  // Tardanzas/salidas anticipadas — mismo patrón: reemplazar por legajo+período+empresa
  if (tardanzas.length) {
    await fetch(`${SUPABASE_URL}/rest/v1/rrhh_tardanzas_salidas?periodo=eq.${periodo}&empresa=eq.${empresa}&legajo=in.(${legajosArchivo.join(',')})`, {
      method: 'DELETE', headers: HDR_SB,
    });
    for (let i = 0; i < tardanzas.length; i += BATCH) {
      const lote = tardanzas.slice(i, i + BATCH);
      const r = await fetch(`${SUPABASE_URL}/rest/v1/rrhh_tardanzas_salidas?on_conflict=legajo,fecha,tipo,empresa`, {
        method: 'POST',
        headers: { ...HDR_JSON, Prefer: 'resolution=merge-duplicates' },
        body: JSON.stringify(lote),
      });
...
export async function renderizarPresentismoCarga(contenedor, alCargar) {
  ...
  async function procesarArchivo(file) {
    ...
    try {
      ...
      if (tipoActivo === 'tango') {
        ...
        // Chequear si ya hay datos para ese período + empresa
        const rEx = await fetch(
          `${SUPABASE_URL}/rest/v1/rrhh_horas_mensual?periodo=eq.${parsed.periodo}&empresa=eq.${parsed.empresa}&select=legajo`,
          { headers: HDR_SB }
        );
  ...
  function mostrarPreviewTango(parsed, periodoExistente) {
    const { mensual, detalle, tardanzas, periodo, empresa } = parsed;
    ...
    estadoDiv.innerHTML = `
      <div class="pres__msg-ok">
        ✓ Detectamos un archivo de <strong>Tango</strong> — <strong>${EMP_LABEL[empresa] ?? empresa}</strong> — <strong>${formatearPeriodo(periodo)}</strong>
  ...
  async function confirmarCargaTango() {
    ...
    try {
      await guardarEnSupabase(datosParseados.mensual, datosParseados.detalle, datosParseados.tardanzas, datosParseados.periodo, datosParseados.empresa);

      estadoDiv.innerHTML = `
        <div class="pres__msg-exito">
          ✓ Datos de <strong>${EMP_LABEL[datosParseados.empresa] ?? datosParseados.empresa}</strong> — <strong>${formatearPeriodo(datosParseados.periodo)}</strong> guardados.
          ${datosParseados.mensual.length} empleados · ${datosParseados.detalle.length.toLocaleString('es-AR')} registros de detalle.
        </div>
      `;
      previewDiv.style.display = 'none';
      fileInput.value = '';

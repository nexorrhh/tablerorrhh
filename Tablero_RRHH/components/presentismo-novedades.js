...
async function renderModoTango(contenedor) {
  ...
  // Flechas de teclado — solo cuando el foco no está en un input y el modo "Tango" está
  // visible (si el usuario pasó a "Chequeo del día", este contenedor queda oculto pero
  // sigue en el DOM, así que sin el chequeo de `hidden` las flechas seguirían navegando
  // por debajo sin que se vea nada).
  const onKeydown = e => {
    if (contenedor.hidden || document.activeElement === inputFecha) return;

...
export async function renderizarPostulantesCandidatos(contenedor) {
  ...
  // Solicitudes aprobadas sin cubrir (para el dropdown de vinculación)
  try {
    solicitudes = await sbGet('solicitudes_personal?select=id,puesto,empresa,cantidad,estado_busqueda&estado=eq.aprobado&estado_busqueda=neq.cubierto&order=created_at.desc');
  } catch (_) {
    solicitudes = [];
  }
  ...
  // Cargar también las búsquedas ya vinculadas a candidatos existentes (pueden estar cubiertas)
  const vinculoIds = [...new Set(candidatos.filter(c => c.solicitud_id).map(c => c.solicitud_id))].filter(id => !solMap[id]);
  if (vinculoIds.length) {
    try {
      const extra = await sbGet(`solicitudes_personal?id=in.(${vinculoIds.join(',')})&select=id,puesto,empresa,cantidad,estado_busqueda`);
      extra.forEach(s => { solMap[s.id] = s; });
    } catch (_) {}
  }
  ...
  function abrirDetalle(id) {
    ...
    detalleEl.innerHTML = `
      ...
      ${c.estado === 'contratado' ? `
        <div class="cand__vincular" id="cand-vincular">
          <h4 class="cand__vincular-titulo">Búsqueda que cubre esta contratación</h4>
          ${c.solicitud_id && solMap[c.solicitud_id] ? `
            <p class="cand__vincular-ok">
              Vinculado a: <strong>${solMap[c.solicitud_id].puesto} — ${solMap[c.solicitud_id].empresa}</strong>
              ${solMap[c.solicitud_id].estado_busqueda === 'cubierto' ? '<span class="cand__vincular-cerrada">· Búsqueda cubierta ✓</span>' : '<span class="cand__vincular-activa">· Vacante parcialmente cubierta</span>'}
            </p>
          ` : solicitudes.length ? `
            <p class="cand__vincular-desc">¿Esta contratación cubre una búsqueda activa? Vinculala para marcarla como cubierta.</p>
            <div class="cand__vincular-row">
              <select class="cand__input" id="cand-sol-sel">
                <option value="">— Elegí una búsqueda —</option>
                ${solicitudes.map(s => `<option value="${s.id}">${s.puesto} — ${s.empresa} (${s.cantidad??'?'} vacante${s.cantidad!==1?'s':''})</option>`).join('')}
              </select>
              <button class="cand__btn-vincular" id="cand-btn-vincular" type="button">Vincular y cerrar búsqueda</button>
            </div>
          ` : `
            <p class="cand__vincular-desc">No hay búsquedas activas para vincular en este momento.</p>
          `}
        </div>
      ` : ''}
    `;
    ...
    form.addEventListener('submit', async e => {
      ...
      try {
        ...
        actualizarLista();
        // Re-render la ficha para reflejar el nuevo estado (incluyendo sección vincular si pasó a contratado)
        msgEl.textContent = 'Guardado.';
        msgEl.classList.add('cand__guardado-msg--ok');
        setTimeout(() => abrirDetalle(id), 700);
      } catch (_) {
    ...
    // ── Vincular a búsqueda activa (solo cuando estado = contratado) ────────────
    const btnVincular = detalleEl.querySelector('#cand-btn-vincular');
    if (btnVincular) {
      btnVincular.addEventListener('click', async () => {
        const solId = detalleEl.querySelector('#cand-sol-sel')?.value;
        if (!solId) { alert('Elegí una búsqueda para vincular.'); return; }
        btnVincular.disabled = true;
        btnVincular.textContent = 'Guardando…';
        try {
          await sbPatch(`candidatos?id=eq.${id}`, { solicitud_id: solId });
          c.solicitud_id = solId;
          // Contar cuántos contratados ya cubren esta búsqueda (incluye el recién vinculado)
          const yaContratados = await sbGet(`candidatos?solicitud_id=eq.${solId}&estado=eq.contratado&select=id`);
          const solData = solicitudes.find(s => s.id === solId) || solMap[solId];
          if (yaContratados.length >= (solData?.cantidad ?? 1)) {
            await sbPatch(`solicitudes_personal?id=eq.${solId}`, { estado_busqueda: 'cubierto' });
            if (solData) solData.estado_busqueda = 'cubierto';
            // Remover del dropdown pero mantener en solMap para mostrar en la ficha
            const idxSol = solicitudes.findIndex(s => s.id === solId);
            if (idxSol !== -1) solicitudes.splice(idxSol, 1);
          }
          abrirDetalle(id);
        } catch (err) {
          alert('Error al vincular: ' + err.message);
          btnVincular.disabled = false;
          btnVincular.textContent = 'Vincular y cerrar búsqueda';
        }
      });
    }

...
export async function renderizarHorasCruce(contenedor) {
  ...
  let rawTango = [], rawOT = [];
  try {
    [rawTango, rawOT] = await Promise.all([
      fetchTodasFilas(`${SUPABASE_URL}/rest/v1/rrhh_horas_mensual?select=legajo,apellido,nombre,departamento,empresa,periodo,hs_normales,hs_extra50,hs_extra100,hs_vac_trabajadas`),
      fetchTodasFilas(`${SUPABASE_URL}/rest/v1/horas_ot_detalle?select=legajo,empresa,periodo,fecha,nombre,horas,anomalo`),
    ]);
  } catch {
  ...
  try {
    rawTangoDetalle = await fetchTodasFilas(
      `${SUPABASE_URL}/rest/v1/rrhh_horas_detalle?select=periodo,fecha&periodo=in.(${periodosComunes.join(',')})`
    );
  ...
  function tablaHtml(filas) {
    return `
      <table class="pind__res-tabla">
        <thead><tr>
          <th class="pind__res-th">Legajo</th>
          <th class="pind__res-th">Empleado</th>
          <th class="pind__res-th">Empresa</th>
          <th class="pind__res-th">Sector</th>
          <th class="pind__res-th" style="text-align:right">Hs. Tango</th>
          <th class="pind__res-th" style="text-align:right">Hs. Capataz</th>
          <th class="pind__res-th" style="text-align:right">Diferencia</th>
        </tr></thead>
        <tbody>
          ${filas.map(f => `
            <tr>

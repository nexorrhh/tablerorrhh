...
export async function renderizarHorasCruce(contenedor) {
  ...
  // Fecha máxima cargada en cada sistema por período Y POR EMPRESA (no un máximo global — dos
  // empresas casi nunca cierran Tango el mismo día, así que si una está completa y la otra no,
  // un máximo global tapa justo la que está atrasada), para poder avisar cuando la diferencia
  // sea simplemente porque a Tango todavía le faltan cerrar los últimos días del mes (algo que
  // pasa naturalmente cuando el mes en curso no cerró todavía, no es un error real).
  let rawTangoDetalle = [];
  try {
    rawTangoDetalle = await fetchTodasFilas(
      `${SUPABASE_URL}/rest/v1/rrhh_horas_detalle?select=periodo,fecha,legajo&periodo=in.(${periodosComunes.join(',')})&order=id.asc`
    );
  } catch { /* el aviso de corte de mes es un plus — si falla, seguimos sin él */ }
  // rrhh_horas_detalle no guarda empresa — se resuelve por legajo cruzando contra rrhh_horas_mensual
  // (mismo criterio que se usa para armar tangoPorClave más abajo).
  const empresaPorLegajoPeriodo = new Map(); // `${legajo}|${periodo}` -> empresa
  rawTango.forEach(t => empresaPorLegajoPeriodo.set(`${t.legajo}|${t.periodo}`, t.empresa));

  const tangoMaxFecha = new Map();  // `${periodo}|${empresa}` -> 'YYYY-MM-DD'
  rawTangoDetalle.forEach(d => {
    if (!d.fecha) return;
    const empresa = empresaPorLegajoPeriodo.get(`${d.legajo}|${d.periodo}`);
    if (!empresa) return;
    const key = `${d.periodo}|${empresa}`;
    if (!tangoMaxFecha.has(key) || d.fecha > tangoMaxFecha.get(key)) tangoMaxFecha.set(key, d.fecha);
  });
  const otMaxFecha = new Map(); // `${periodo}|${empresa}` -> 'YYYY-MM-DD'
  rawOT.forEach(d => {
    if (!d.fecha) return;
    const key = `${d.periodo}|${d.empresa}`;
    if (!otMaxFecha.has(key) || d.fecha > otMaxFecha.get(key)) otMaxFecha.set(key, d.fecha);
  });
  ...
  function render() {
    ...
    // Chequeo por empresa — ver comentario donde se arman tangoMaxFecha/otMaxFecha.
    const cortesPorEmpresa = Object.keys(EMP_LABEL).map(emp => {
      const tMax = tangoMaxFecha.get(`${periodoActivo}|${emp}`);
      const oMax = otMaxFecha.get(`${periodoActivo}|${emp}`);
      const dias = tMax && oMax && oMax > tMax ? Math.round((new Date(oMax) - new Date(tMax)) / 86400000) : 0;
      return { emp, tMax, oMax, dias };
    }).filter(c => c.dias >= 2 && (!filtroEmpresa || filtroEmpresa === c.emp));
    ...
    contenedor.innerHTML = `
      <div class="pres__personas-wrap">
        ...
        ${cortesPorEmpresa.map(c => `
        <div class="pres__aviso-parcial">
          Tango de <strong>${EMP_LABEL[c.emp]}</strong> en ${fmtPeriodo(periodoActivo)} tiene datos cargados hasta el
          <strong>${fmtFecha(c.tMax)}</strong>, mientras que Capataz llega hasta el <strong>${fmtFecha(c.oMax)}</strong>
          (${c.dias} día${c.dias !== 1 ? 's' : ''} de diferencia). Si el mes en Tango todavía no cerró para esta empresa,
          buena parte de las diferencias de este período pueden deberse a esos días pendientes de cerrar, no a un
          error real — conviene repetir el cruce cuando Tango tenga el mes completo para ${EMP_LABEL[c.emp]}.
        </div>`).join('')}

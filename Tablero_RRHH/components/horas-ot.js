...
async function cargarFilasOT() {
  const rows = await fetchTodasFilas(`${SUPABASE_URL}/rest/v1/horas_ot_detalle?select=${SELECT_OT}&order=periodo.asc,id.asc`);
...
export async function renderizarHorasOT(contenedor) {
  ...
  // Duración del fundido al cambiar de mes en "Desglose" — tiene que coincidir con la
  // transición de opacidad definida en .hot__panel-fade (componentes.css).
  const DURACION_FADE_PANEL = 150;
  ...
  function wireInforme(pers) {
    ...
    sel?.addEventListener('change', () => {
      ...
      panel.classList.add('hot__panel-fade');
      setTimeout(() => {
        panel.innerHTML = panelHtml();
        wirePanel();
        dibujarGrafico(pers);
        requestAnimationFrame(() => panel.classList.remove('hot__panel-fade'));
      }, DURACION_FADE_PANEL);
    });
    wirePanel();
  }

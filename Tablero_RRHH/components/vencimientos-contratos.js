...
const TIPOS = [
  'Monotributista', 'Contrato plazo fijo',
  'Locación de servicios', 'Contrato de obra', 'Otro',
];
...
export async function renderizarVencimientosContratos(contenedor) {
  ...
  function renderVista() {
    ...
    contenedor.innerHTML = `
      <div class="venc">
        ...
        ${registros.length === 0
          ? `<p class="venc__vacio">No hay contratos. Agregá el primero.</p>`
  ...
  function abrirModal(reg = null) {
    ...
    modal.innerHTML = `
      <div class="venc__modal" role="dialog" aria-modal="true">
        <h3 class="venc__modal-titulo">${reg ? 'Editar contrato' : 'Nuevo contrato'}</h3>

        <label class="venc__label">Nombre del contratista *</label>
        <input  class="venc__input" id="vm-nombre"  type="text"  value="${e(reg?.nombre)}"  placeholder="Ej: García Juan">

        <div class="venc__row2">
          <div>
            <label class="venc__label">Tipo *</label>
            <select class="venc__input" id="vm-tipo">
              ${TIPOS.map(t => `<option ${reg?.tipo===t?'selected':''}>${t}</option>`).join('')}
            </select>
          </div>
          <div>
        ...
        <input class="venc__input" id="vm-area" type="text" value="${e(reg?.area)}" placeholder="Ej: Producción">
  ...
  async function guardar(cerrar) {
    ...
    try {
      const body = { nombre, tipo, empresa, area, fecha_inicio: inicio, fecha_vencimiento: venc, notas };
      editandoId ? await sbUpdate(editandoId, body) : await sbInsert({ ...body, activo: true });
      cerrar();
      await cargar();
      toast(editandoId ? 'Contrato actualizado' : 'Contrato agregado', 'ok');
    } catch (err) {
  ...
  async function eliminar(id) {
    ...
    try { await sbDelete(id); await cargar(); toast('Contrato eliminado'); }
...
function filaContrato(r) {
  ...
  return `
    <tr class="${rowCls}">
      <td class="venc__td-bold">${e(r.nombre)}</td>
      <td>${e(r.tipo)}</td>

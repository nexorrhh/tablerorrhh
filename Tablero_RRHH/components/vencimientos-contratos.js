...
export async function renderizarVencimientosContratos(contenedor) {
  ...
  function renderVista() {
    ...
    // Cruzar monotributistas con sus registros por legajo — se normaliza a string en los dos
    // lados porque Supabase puede devolver el legajo como número de un lado y texto del otro.
    const mapContratos = new Map(); // legajo (string) → registro de contrato
    const mapSeguros   = new Map(); // legajo (string) → registro de seguro
    registros.forEach(r => {
      if (!r.legajo) return;
      esSeguro(r.tipo) ? mapSeguros.set(String(r.legajo), r) : mapContratos.set(String(r.legajo), r);
    });

    const monoHtml = monotribList.map(emp => {
      const contrato = mapContratos.get(String(emp.legajo));
      const seguro   = mapSeguros.get(String(emp.legajo));
    ...
    // Lo que ya se muestra arriba (cruzado por legajo) no se repite acá abajo.
    const legajosMonotrib = new Set(monotribList.map(emp => String(emp.legajo)));
    const registrosOtros  = registros.filter(r => !r.legajo || !legajosMonotrib.has(String(r.legajo)));
    ...
    contenedor.innerHTML = `
      <div class="venc">
        ...
        <div class="venc__seccion">
          <h4 class="venc__seccion-h">Otros contratos y seguros</h4>
          ${registrosOtros.length === 0
            ? `<p class="venc__vacio">No hay más registros — el resto ya está arriba, cruzado con monotributistas.</p>`
            : `<div class="venc__tabla-wrap">
                <table class="venc__tabla">
                  <thead><tr>
                  ...
                  <tbody>${registrosOtros.map(filaContrato).join('')}</tbody>
                </table>
              </div>`}
        </div>
      </div>`;
  ...
  function abrirModal(reg = null, prefill = null) {
    ...
    modal.innerHTML = `
      <div class="venc__modal" role="dialog" aria-modal="true">
        ...
        <label class="venc__label">Persona (si es un monotributista)</label>
        <select class="venc__input" id="vm-legajo">
          <option value="">— No corresponde a un monotributista —</option>
          ${monotribList.map(emp => `<option value="${emp.legajo}" ${String(reg?.legajo ?? prefill?.legajo ?? '') === String(emp.legajo) ? 'selected' : ''}>${e(emp.apellido_y_nombre)} · #${emp.legajo}</option>`).join('')}
        </select>
        <p class="venc__ayuda">Elegilo para que este registro se cruce con la tabla "Monotributistas activos" de arriba. Si no lo elegís (o quedó mal cargado antes), no va a aparecer tildado ahí aunque esté cargado acá abajo.</p>
    ...
    // Al elegir una persona, completa el nombre solo (si todavía está vacío)
    modal.querySelector('#vm-legajo').addEventListener('change', ev => {
      const persona = monotribList.find(p => String(p.legajo) === ev.target.value);
      const nombreInput = modal.querySelector('#vm-nombre');
      if (persona && !nombreInput.value.trim()) nombreInput.value = persona.apellido_y_nombre;
    });

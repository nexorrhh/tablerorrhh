...
export async function renderizarVencimientosPagos(contenedor) {
  ...
  function renderVista() {
    ...
    contenedor.querySelector('#pg-filtro-texto').addEventListener('input', ev => {
      filtroTexto = ev.target.value;
      const cursor = ev.target.selectionStart;
      renderVista();
      // renderVista() reconstruye todo el HTML (incluido este input) — sin esto,
      // el campo perdería el foco en cada letra y habría que hacer clic de nuevo.
      const inputNuevo = contenedor.querySelector('#pg-filtro-texto');
      if (inputNuevo) { inputNuevo.focus(); inputNuevo.setSelectionRange(cursor, cursor); }
    });

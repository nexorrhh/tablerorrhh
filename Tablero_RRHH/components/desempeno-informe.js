...
function dibujarInformeAnual(pdf, { persona, tipo, anio, datos }) {
  ...
  const calc = tipo === 'mensual'
    ? calcularMensual({ ausentismo_dias: datos.ausentismo_dias, tardanzas_cant: datos.tardanzas_cant, evaluacion_aptitudinal: datos.evaluacion_aptitudinal || 0, evaluacion_operativa: datos.evaluacion_operativa || 0 })
    : calcularQuincenal({ ausentismo_dias: datos.ausentismo_dias, tardanzas_cant: datos.tardanzas_cant, epp_nivel: datos.epp_nivel || 0, reprocesos_nivel: datos.reprocesos_nivel || 0, evaluacion_aptitudinal: datos.evaluacion_aptitudinal || 0, evaluacion_operativa: datos.evaluacion_operativa || 0 });
  const { pAus, pTard, pEpp, pRepro, pEval } = calc.desglose;
  const maxAus = tipo === 'mensual' ? 25 : 10;
  const maxTard = tipo === 'mensual' ? 15 : 5;
  const maxEval = tipo === 'mensual' ? 60 : 50;
  const hayEval = datos.evaluacion_aptitudinal != null && datos.evaluacion_operativa != null;

  const filas = [
    ['Ausentismo', `${datos.ausentismo_dias} día(s)`, `${pAus.toFixed(1)} / ${maxAus}`],
    ['Llegadas tarde / salidas anticipadas', `${datos.tardanzas_cant}`, `${pTard.toFixed(1)} / ${maxTard}`],
  ];
  if (tipo === 'quincenal') {
    filas.push(
      ['Cumplimiento de uso de EPP', labelNivel(datos.epp_nivel), celdaPuntos(datos.epp_nivel, pEpp, 15)],
      ['Calidad de trabajo (reprocesos)', labelNivel(datos.reprocesos_nivel), celdaPuntos(datos.reprocesos_nivel, pRepro, 10)],
    );
  }
  filas.push(
    ['Evaluación aptitudinal', labelNivel(datos.evaluacion_aptitudinal), ''],
    ['Evaluación operativa', labelNivel(datos.evaluacion_operativa), ''],
    ['Evaluación del superior (promedio aptitudinal/operativa)', '', celdaPuntos(hayEval ? 1 : null, pEval, maxEval)],
  );
...
export function generarInformeAnualPDF({ persona, tipo, anio, datos }) {
  if (librerianoDisponible()) return;
  const { jsPDF } = window.jspdf;
  const pdf = new jsPDF('p', 'mm', 'a4');
  dibujarInformeAnual(pdf, { persona, tipo, anio, datos });

...
export async function renderizarPresentismoPersonas(contenedor) {
  ...
  function renderTabla(datos) {
    ...
    tablaWrap.querySelector('#pres-tbody').addEventListener('click', async e => {
      ...
      detRow.innerHTML = `<td colspan="8"><div class="pres__det-loading">Cargando detalle…</div></td>`;
  ...
  function renderDetalle(detalle, persona) {
    ...
    function calcularDia(filas) {
      ...
      // balance = tiempo real en el trabajo - tiempo esperado
      // positivo → sobrecumplió (llegó antes / se fue después, sin ser hora extra formal)
      // negativo → déficit (llegó tarde / salió antes)
      const balance = hs_esp > 0 ? +(hs_reales - hs_esp).toFixed(2) : null;
    ...
    const balTotal = hayEsperadas ? +(totalReales - totalEsp).toFixed(2) : null;
    ...
    const tarjetaAnalitica = `
      <div class="pres__anal-grid">
        ...
        <div class="pres__anal-item pres__anal-item--dest ${balTotal > 0 ? 'pres__anal-item--pos' : balTotal < -0.5 ? 'pres__anal-item--neg' : ''}">
          ...
          <span class="pres__anal-sub">${hmin(totalReales)} reales de ${hmin(totalEsp)} esperadas</span>
        </div>
` : ''}
    ...
    // ── Calendario visual ─────────────────────────────────────────────────────
    const iconos = { ok: '✓', extra: '+', parcial: '~', justif: 'J', falta: '✗' };

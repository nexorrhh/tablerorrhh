...
// Barra segmentada con las horas de Capataz de ese día, una porción por OT/tarea. El detalle
// (a qué OT se imputó cada franja) se ve en un tooltip propio armado en JS — ver mostrarTooltipOT
// más abajo — en vez del title nativo del navegador, que es lento y no se puede estilar.
function celdaOT(fecha, segmentos) {
  ...
  return `<div class="pres__ot-bar" data-ot-tip="${datos}">${partes}</div>`;
}
...
function habilitarTooltipsOT(raiz) {
  raiz.addEventListener('mouseover', e => {
    const barra = e.target.closest('.pres__ot-bar');
    if (barra) mostrarTooltipOT(barra, e.clientX, e.clientY);
  });
  raiz.addEventListener('mousemove', e => {
    const barra = e.target.closest('.pres__ot-bar');
    if (barra && tooltipOT?.classList.contains('pres__ot-tooltip--visible')) {
      posicionarTooltipOT(tooltipOT, e.clientX, e.clientY);
    }
  });
  raiz.addEventListener('mouseout', e => {
    const barra = e.target.closest('.pres__ot-bar');
    if (barra && !barra.contains(e.relatedTarget)) ocultarTooltipOT();
  });
}
...
export async function renderizarPresentismoPersonas(contenedor) {
  ...
  function renderDetalle(detalle, persona, eventos = [], otDelMes = []) {
    ...
    function calcularDia(filas, fecha) {
      ...
      // Sin aceptar los dos códigos acá, el detalle diario de Comoing perdía la hora extra
      // (no aparecía en la columna "Extra 50%" ni sumaba al total, aunque sí se contaba bien
      // en el acumulado mensual de rrhh_horas_mensual).
      const ext50  = filas.filter(f => f.tipo_hora === 'HSEXT' || f.tipo_hora === 'HSEXT50');

...
const CATEGORIA_ICONO = { enfermedad: 'E', accidente: 'Ac', licencia: 'Li', aviso: 'Av', sin_aviso: 'SA', sin_clasificar: '?' };
const CATEGORIA_COLOR = {
  enfermedad: '#d97706', accidente: '#dc2626', licencia: '#0891b2', aviso: '#2563eb',
  sin_aviso: '#991b1b', sin_clasificar: '#94a3b8',
};
const CATEGORIAS_LEYENDA = ['enfermedad', 'accidente', 'licencia', 'aviso', 'sin_aviso', 'sin_clasificar'];
...
export function formatearPeriodo(p) {
  const [y, m] = p.split('-');
  return `${MESES[+m - 1]} ${y}`;
}
...
// se puede estilar.
function celdaOT(fecha, segmentos, horasAnomalas = 0) {
  const avisoAnomalo = horasAnomalas > 0
    ? `<span class="hotc__tag hotc__tag--warn" title="Además tiene ${horasAnomalas.toFixed(1)}h cargadas en Capataz con más de 16h en un solo día — excluidas de este total por ser casi seguro un error de carga">+${horasAnomalas.toFixed(1)}h anóm.</span>`
    : '';
  if (!segmentos || !segmentos.length) {
    return horasAnomalas > 0
      ? `<span class="pres__ot-vacio">—</span> ${avisoAnomalo}`
      : '<span class="pres__ot-vacio">—</span>';
  }
  const total = segmentos.reduce((s, o) => s + o.horas, 0);
  if (total <= 0) return `<span class="pres__ot-vacio">—</span> ${avisoAnomalo}`;
...
export function habilitarTooltipsOT(raiz) {
...
export async function renderizarPresentismoPersonas(contenedor) {
  ...
  function renderTabla(datos) {
    ...
    tablaWrap.querySelector('#pres-tbody').addEventListener('click', e => {
      const fila = e.target.closest('.pres__fila-emp');
    ...
    if (filtroTexto.trim() && filasVisibles.length === 1) {
      abrirDetalle(tablaWrap.querySelector('.pres__fila-emp'));
    }
  }

  // Anima el despliegue de una fila de detalle ya insertada, fijando su altura real
  // (el contenido puede crecer después, ej. al pasar de "Cargando…" al detalle real).
  function expandirColapsable(el) {
    requestAnimationFrame(() => requestAnimationFrame(() => {
      el.style.maxHeight = el.scrollHeight + 'px';
      el.classList.add('pres__det-collapse--abierto');
    }));
  }

  // Anima el cierre de una fila de detalle y recién la saca del DOM al terminar la transición.
  function cerrarFilaDetalle(detRow) {
    if (!detRow) return;
    const el = detRow.querySelector('.pres__det-collapse');
    if (!el) { detRow.remove(); return; }
    el.style.maxHeight = el.scrollHeight + 'px';
    el.classList.remove('pres__det-collapse--abierto');
    requestAnimationFrame(() => requestAnimationFrame(() => {
      el.style.maxHeight = '0px';
    }));
    setTimeout(() => detRow.remove(), 340);
  }

  async function abrirDetalle(fila) {
      if (!fila) return;
      const btn = fila.querySelector('.pres__btn-exp');
      const legajo  = +btn.dataset.legajo;
      const filaId  = `pres-det-${legajo}`;
      const existente = tablaWrap.querySelector(`#${filaId}`);

      if (existente) {
        cerrarFilaDetalle(existente);
        btn.textContent = '▶';
        btn.setAttribute('aria-expanded', 'false');
        return;
      }

      // Cerrar cualquier otra fila expandida (con la misma animación)
      cerrarFilaDetalle(tablaWrap.querySelector('.pres__fila-det'));
      ...
      detRow.innerHTML = `<td colspan="11"><div class="pres__det-collapse"><div class="pres__det-loading">Cargando detalle…</div></div></td>`;
      fila.insertAdjacentElement('afterend', detRow);

      const collapseEl = detRow.querySelector('.pres__det-collapse');
      expandirColapsable(collapseEl);
      ...
      if (detalle === undefined || eventos === undefined || otDelMes === undefined) {
        try {
          const [rDet, rEv, rOt] = await Promise.all([
            ...
            // Se trae también lo anómalo (no solo anomalo=eq.false): renderDetalleDia lo separa
            // y lo muestra como aviso en vez de descartarlo en silencio (ver celdaOT).
            persona?.empresa
              ? fetch(`${SUPABASE_URL}/rest/v1/horas_ot_detalle?legajo=eq.${legajo}&periodo=eq.${periodoActivo}&empresa=eq.${persona.empresa}&select=fecha,ot,operacion,horas,anomalo`, { headers: HDR })
      ...
      detRow.querySelector('td').setAttribute('colspan', '11');
      collapseEl.innerHTML = renderDetalle(detalle, persona, eventos, otDelMes);
      // El detalle real suele ser más alto que el "Cargando…" — recalcular para que
      // termine de desplegarse hasta la altura correcta en vez de cortarse de golpe.
      expandirColapsable(collapseEl);
  }
  ...
  function renderDetalle(detalle, persona, eventos = [], otDelMes = []) {
    return renderDetalleDia(detalle, persona, eventos, otDelMes, tipoDe(persona), periodoActivo, mapaAusencias);
  }

  cargarYMostrar();
}

// Detalle diario (calendario + tabla) de una persona en un período — lo usan tanto "Por
// persona" (arriba) como Horas y Presentismo → Ficha individual (presentismo-ficha.js).
// tipo: 'mensual' | 'quincenal' | 'sin_asignar' — define el criterio de balance (ver abajo).
export function renderDetalleDia(detalle, persona, eventos = [], otDelMes = [], tipo, periodoActivo, mapaAusencias) {
    ...
    // Horas por OT (Capataz) del día — cada fecha puede tener más de una OT/tarea. Las cargas
    // "anómalas" (más de 16h en un día — casi seguro un error de tipeo en Capataz, ver
    // horas-ot.js) se muestran aparte con un aviso en vez de sumarlas al total del día: si se
    // filtraran en silencio como antes, alguien podía ver en Cruce de Horas que a esta persona
    // le falta una carga anómala y no encontrar ningún rastro de eso acá.
    const otPorFecha = new Map();
    const otAnomaloPorFecha = new Map();
    otDelMes.forEach(o => {
      if (o.anomalo) {
        otAnomaloPorFecha.set(o.fecha, (otAnomaloPorFecha.get(o.fecha) || 0) + (+o.horas || 0));
        return;
      }
    ...
    // Día donde Capataz tiene horas cargadas por OT pero Tango no tiene ningún registro —
    // sin esto, el día directamente no aparecía en la tabla (los días salen de porFecha, que
    // sale de Tango), así que un hueco de carga en Tango quedaba invisible en vez de avisar
    // que falta cargar ese día. Todas las columnas de Tango van en "—" (ver n()/hmin de más
    // abajo, que ya devuelven "—" para 0) y la de Capataz sí muestra el dato real.
    function diaSinTango() {
      return {
        hs_reales: 0, hs_norm: 0, hs_just: 0, hs_nojust: 0, hs_e50: 0, hs_e100: 0, hs_esp: 0,
        vac_esperadas: 0, vac_trabajadas: 0, balance: null, descJust: '',
        estado: 'sin_tango', estadoLabel: 'Sin datos de Tango', categoria: null,
      };
    }

    const dias = [...new Set([...porFecha.keys(), ...otPorFecha.keys()])].sort();

    // Pre-calcular todos los días para las métricas del resumen
    const calculos = new Map(dias.map(f => [f, porFecha.has(f) ? calcularDia(porFecha.get(f), f) : diaSinTango()]));
...
` : ''}
    ...
    // ── Calendario visual ─────────────────────────────────────────────────────
    const iconos = { ok: '✓', extra: '+', parcial: '~', justif: 'J', falta: '✗', libre: '·', vacaciones: 'V', vactrab: 'VT', sin_tango: '!' };
    ...
    const filasTbl = dias.map(fecha => {
      ...
      const estadoBadge = {
        ...
        sin_tango:  `<span class="pres__det-est pres__det-est--sin_tango" title="Capataz tiene horas cargadas este día pero Tango no tiene ningún registro — falta cargar este día en Tango">! Falta cargar en Tango</span>`,
      ...
      return `
        <tr class="pres__det-tr pres__det-tr--${estado}">
          ...
          <td class="pres__det-td pres__det-td--ot">${celdaOT(diaYMes(fecha), otPorFecha.get(fecha), otAnomaloPorFecha.get(fecha) || 0)}</td>
        </tr>
      `;
    }).join('');
    ...
    return `
      <div class="pres__det-inner">
        ...
        <div class="pres__cal-leyenda">
          ...
          <span class="pres__cal-leg pres__cal-leg--sin_tango">! Falta cargar en Tango</span>

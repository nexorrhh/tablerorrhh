...
import { RESULTADO_LABEL, RESULTADO_COLOR, corresponde } from './desempeno-cargar.js';
...
export async function renderizarDesempenoIndicadores(contenedor) {
  ...
  let empleados = [], mapaClasif = new Map(), filasPrueba = [];
  try {
    const [rE, mc, rP] = await Promise.all([
  ...
  function render(filasEval) {
    ...
    const universo = empleados.filter(p => corresponde(p, anioActivo, mapaClasif));
  ...
  function renderPrueba() {
    ...
    const universo = activosIngreso.filter(p => {
      ...
      if (evalPorClave.has(clave)) return true;
  ...
  function exportarExcel({ anio, evaluados, pendientes, evalPorClave }, resumenPrueba) {
    ...
    const ws1 = XLSX.utils.aoa_to_sheet([
      ['Legajo', 'Nombre', 'Empresa', 'Sector', 'Tipo', 'Ausentismo (días)', 'Tardanzas', 'Faltas EPP', 'Reprocesos', 'Evaluación superior', 'Puntaje', 'Resultado', 'Aspecto destacable', 'Aspecto a mejorar'],
      ...evaluados.map(p => {
        const f = evalPorClave.get(`${p.legajo}|${p.empresa}`);
        return [p.legajo, p.apellido_y_nombre, EMP_LABEL[p.empresa] || p.empresa, p.desc_puesto || '', TIPO_LABEL[f.tipo] || f.tipo,
          f.ausentismo_dias, f.tardanzas_cant ?? '', f.epp_faltas ?? '', f.reprocesos ?? '', f.evaluacion_superior,
          f.puntaje, RESULTADO_LABEL[f.resultado] || f.resultado, f.aspecto_destacable || '', f.aspecto_mejorar || ''];
      }),
    ]);
    ws1['!cols'] = [10, 26, 12, 20, 10, 14, 10, 10, 10, 14, 10, 12, 24, 24].map(w => ({ wch: w }));

...
import { RESULTADO_LABEL, RESULTADO_COLOR, corresponde, esReingreso, aniosDisponiblesDesempeno, PRIMER_ANIO_CON_DATOS_TANGO } from './desempeno-cargar.js';
...
export async function renderizarDesempenoIndicadores(contenedor) {
  ...
  function render(filasEval) {
    ...
    // 2025 no es un ciclo real de PG-6.01 (se habilitó solo como referencia manual, para poder
    // "copiar" del año anterior) — exigirle antigüedad ≥365 días no tiene sentido ahí, así que
    // se cuenta a cualquiera que tenga una evaluación cargada, sin universo de "corresponde" ni
    // pendientes. De PRIMER_ANIO_CON_DATOS_TANGO en adelante sigue el criterio de siempre.
    const esAnioReferencia = anioActivo < PRIMER_ANIO_CON_DATOS_TANGO;
    const universo    = esAnioReferencia
      ? empleados.filter(p => evalPorClave.has(`${p.legajo}|${p.empresa}`))
      : empleados.filter(p => corresponde(p, anioActivo, mapaClasif, manualSet));
    const evaluados  = esAnioReferencia ? universo : universo.filter(p => evalPorClave.has(`${p.legajo}|${p.empresa}`));
    const pendientes = esAnioReferencia ? [] : universo.filter(p => !evalPorClave.has(`${p.legajo}|${p.empresa}`));

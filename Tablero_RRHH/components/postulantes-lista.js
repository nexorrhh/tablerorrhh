...
export async function renderizarPostulantesLista(contenedor) {
  ...
  _mPresel.innerHTML = `
    <div class="plista__modal plista__modal--eval" role="dialog" aria-modal="true" aria-labelledby="plista-presel-titulo">
      ...
      <div class="plista__eval-campo">
        <label class="plista__eval-label" for="plista-puesto-eval">Puesto evaluado</label>
        <select class="plista__eval-input" id="plista-puesto-eval">
          <option value="">— Seleccioná un puesto —</option>
        </select>
      </div>
      <div class="plista__eval-campo">
        <label class="plista__eval-label">Calificación general</label>
        <div class="plista__estrellas" id="plista-estrellas" role="group" aria-label="Calificación del 1 al 5">
          <button type="button" class="plista__estrella" data-val="1" aria-label="1 estrella">★</button>
          <button type="button" class="plista__estrella" data-val="2" aria-label="2 estrellas">★</button>
          <button type="button" class="plista__estrella" data-val="3" aria-label="3 estrellas">★</button>
          <button type="button" class="plista__estrella" data-val="4" aria-label="4 estrellas">★</button>
          <button type="button" class="plista__estrella" data-val="5" aria-label="5 estrellas">★</button>
        </div>
        <input type="hidden" id="plista-calificacion" value="">
      </div>
      <div class="plista__eval-campo">
        <label class="plista__eval-label" for="plista-exp-rol">Experiencia en el rol</label>
        <select class="plista__eval-input" id="plista-exp-rol">
      ...
      <div class="plista__eval-campo">
        <label class="plista__eval-label" for="plista-formacion">Formación / Certificación</label>
        <select class="plista__eval-input" id="plista-formacion">
          <option value="">— Seleccioná —</option>
      ...
      <div class="plista__eval-campo">
        <label class="plista__eval-label" for="plista-estab-lab">Estabilidad laboral</label>
        <select class="plista__eval-input" id="plista-estab-lab">
          <option value="">— Seleccioná —</option>
      ...
      <div class="plista__eval-campo">
        <label class="plista__eval-label" for="plista-notas">Observaciones</label>
        <textarea class="plista__eval-textarea" id="plista-notas" rows="3"
                  placeholder="Notas internas sobre el perfil (opcional)"></textarea>
      </div>
      <div class="plista__eval-campo">
        <label class="plista__eval-label" for="plista-presel-por">Preseleccionado por</label>
        <input class="plista__eval-input plista__modal-input--readonly" id="plista-presel-por" type="text"
               autocomplete="off" readonly>
      </div>
      <div class="plista__modal-footer">
        <button class="plista__btn-cancel" type="button" id="plista-presel-cancel">Cancelar</button>
        <button class="plista__btn-ok" type="button" id="plista-presel-ok">Preseleccionar</button>
      </div>
    </div>
  `;
  document.body.appendChild(_mPresel);

  // ── Estrellas para el modal de preselección ──
  const plistaEstrellasEl = document.querySelector('#plista-estrellas');
  const plistaCalInput    = document.querySelector('#plista-calificacion');

  function actualizarPlistaEstrellas(val) {
    plistaCalInput.value = val > 0 ? val : '';
    plistaEstrellasEl.querySelectorAll('.plista__estrella').forEach((b, i) => {
      b.classList.toggle('plista__estrella--activa', i < val);
    });
  }

  plistaEstrellasEl.querySelectorAll('.plista__estrella').forEach(btn => {
    btn.addEventListener('click', () => actualizarPlistaEstrellas(+btn.dataset.val));
    btn.addEventListener('mouseenter', () => {
      const hv = +btn.dataset.val;
      plistaEstrellasEl.querySelectorAll('.plista__estrella').forEach((b, i) => {
        b.classList.toggle('plista__estrella--hover', i < hv);
      });
    });
  });
  plistaEstrellasEl.addEventListener('mouseleave', () => {
    plistaEstrellasEl.querySelectorAll('.plista__estrella').forEach(b => b.classList.remove('plista__estrella--hover'));
  });
  ...
  document.querySelector('#plista-presel-ok').addEventListener('click', async () => {
    ...
    try {
      await sbPost('preseleccionados', {
        ...
        estado:                 'activo',
        puesto_evaluado:        document.querySelector('#plista-puesto-eval').value.trim() || null,
        calificacion:           plistaCalInput.value ? Number(plistaCalInput.value) : null,
        experiencia_rol:        document.querySelector('#plista-exp-rol').value || null,
        formacion:              document.querySelector('#plista-formacion').value || null,
        estabilidad_laboral:    document.querySelector('#plista-estab-lab').value || null,
      });
      ...
      btn.disabled = false;
      btn.textContent = 'Preseleccionar';
    }
  });
    ...
    contenedor.querySelectorAll('.plista__btn-presel').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        const notaPrefill = p.tipo === 'agrupado'
          ...
          : '';
        const puestoSel = document.querySelector('#plista-puesto-eval');
        puestoSel.innerHTML = '<option value="">— Seleccioná un puesto —</option>' +
          sectoresUnicos.map(s => `<option value="${s}">${s}</option>`).join('');
        puestoSel.value = sectores[0] || '';
        actualizarPlistaEstrellas(0);
        document.querySelector('#plista-exp-rol').value    = '';
        document.querySelector('#plista-formacion').value  = '';
        document.querySelector('#plista-estab-lab').value  = '';

...
// Variación del comprobante contra el monto acordado con el proveedor (Proveedores → Acuerdo comercial),
// una referencia más estable que "vs. mes anterior" para proveedores con facturación irregular.
function variacionAcuerdoHtml(importe, montoAcordado) {
  const acordado = Number(montoAcordado) || 0;
  if (!acordado) return '';
  const variacion = ((Number(importe) || 0) - acordado) / acordado * 100;
  const signo = variacion > 0 ? '+' : '';
  const cls = variacion > 0 ? 'arca-aut__var--suba' : variacion < 0 ? 'arca-aut__var--baja' : 'arca-aut__var--igual';
  return `<span class="arca-aut__var ${cls}">${signo}${variacion.toFixed(1)}% vs. acordado</span>`;
}
...
export async function renderizarArcaAutorizar(contenedor) {
  ...
  async function cargar() {
    try {
      const [comps, proveedores, perfiles] = await Promise.all([
        ...
        fetch(`${SUPABASE_URL}/rest/v1/arca_proveedores?select=cuit,autorizador_default_id,fecha_acuerdo,monto_acordado`, { headers: HDR_SB }).then(r => r.ok ? r.json() : []),
      ...
      acuerdoPorCuit = new Map(
        proveedores.filter(p => p.monto_acordado != null).map(p => [p.cuit, { fecha: p.fecha_acuerdo, monto: p.monto_acordado }])
      );
    } catch {
  ...
  function abrirModalRevisar(c) {
    ...
    const variacion = calcularVariacion(c, totales);
    const historialItems = obtenerHistorial(c, comprobantes);
    const acuerdo = acuerdoPorCuit.get(c.cuit);
    ...
    modal.innerHTML = `
      <div class="venc__modal" role="dialog" aria-modal="true">
        ...
        ${autorizadorPorCuit.get(c.cuit) ? `<p class="venc__ayuda">👤 Debería autorizar: <strong>${e(autorizadorPorCuit.get(c.cuit))}</strong></p>` : ''}
        ${acuerdo ? `<p class="venc__ayuda">📄 Monto acordado: <strong>$${Number(acuerdo.monto).toLocaleString('es-AR')}</strong>${acuerdo.fecha ? ' (acordado el ' + fmtFecha(acuerdo.fecha) + ')' : ''} — ${variacionAcuerdoHtml(c.importe_total, acuerdo.monto)}</p>` : ''}

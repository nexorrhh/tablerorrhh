...
export async function renderizarPlantelListado(contenedor) {
  ...
  let empleados;
  try {
    const [rows, mapaClasif] = await Promise.all([
      obtenerTabla('v_empleados_activos', 'legajo,apellido_y_nombre,empresa,desc_puesto'),
      obtenerClasificacionPuestos(),
    ]);
    empleados = rows;
    empleados.forEach(e => {
      e.desc_puesto = normalizarPuesto(e.desc_puesto);
      e.tipo = tipoPuesto(e.desc_puesto, mapaClasif);
    });
    empleados.sort((a, b) => {
  ...
  const tiposConGente = ['mensual', 'quincenal', 'sin_asignar'].filter(t => empleados.some(e => e.tipo === t));
  ...
  contenedor.innerHTML = `
    <div class="plantel">
      ...
      <div class="plantel__filtros">
        ...
        <div class="plantel__empresa-pills" role="group" aria-label="Filtrar por tipo de puesto">
          <button class="plantel__pill plantel__pill--activo" data-tipo="">
            Todos&ensp;<span class="plantel__pill-count">${empleados.length}</span>
          </button>
          ${tiposConGente.map(t => `
          <button class="plantel__pill" data-tipo="${t}">
            ${TIPO_LABEL[t]}&ensp;<span class="plantel__pill-count">${empleados.filter(e => e.tipo === t).length}</span>
          </button>`).join('')}
        </div>
  ...
  const pillsEmpresa  = contenedor.querySelectorAll('[data-empresa]');
  const pillsTipo     = contenedor.querySelectorAll('[data-tipo]');
  ...
  function filtrar() {
    ...
    const visibles = empleados.filter(e => {
      ...
      return okEmpresa && okTipo && okPuesto && okTexto;
    });
  ...
  pillsEmpresa.forEach(btn => {
    btn.addEventListener('click', () => {
      filtroEmpresa = btn.dataset.empresa;
      pillsEmpresa.forEach(b => b.classList.remove('plantel__pill--activo'));
      btn.classList.add('plantel__pill--activo');
      filtrar();
    });
  });

  pillsTipo.forEach(btn => {
    btn.addEventListener('click', () => {
      filtroTipo = btn.dataset.tipo;
      pillsTipo.forEach(b => b.classList.remove('plantel__pill--activo'));
      btn.classList.add('plantel__pill--activo');
      filtrar();
    });
  });

...
import { obtenerReingresosManualesSet, marcarReingresoManual } from '../data/reingresos-manuales.js';
...
export async function renderizarDesempenoPrueba(contenedor) {
  ...
  let empleados = [], manualSet = new Set();
  try {
    const [rE, ms] = await Promise.all([
  ...
  async function renderPendientes() {
    ...
    const pendientes = activos
      .filter(p => !evalSet.has(`${p.legajo}|${p.empresa}`) && !esReingreso(p, manualSet))
  ...
  function renderForm(persona, previa, automaticos) {
    ...
    formEl.innerHTML = `
      <div class="pind__sec">
        ...
        ${!previa && esReingreso(persona, manualSet) ? `
    ...
    formEl.querySelector('#prueba-marcar-reingreso')?.addEventListener('click', async (e) => {
      ...
      try {
        await marcarReingresoManual(persona.legajo, persona.empresa, 'Marcado a mano desde Período de prueba');

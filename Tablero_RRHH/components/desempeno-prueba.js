...
import { resultadoDe, RESULTADO_LABEL, RESULTADO_COLOR, esReingreso } from './desempeno-cargar.js';
...
export async function renderizarDesempenoPrueba(contenedor) {
  ...
  let empleados = [];
  try {
    const rE = await fetch(`${SUPABASE_URL}/rest/v1/empleados?select=legajo,apellido_y_nombre,empresa,desc_puesto,activo,fecha_ingreso,creado_en&limit=2000`, { headers: HDR });
  ...
  async function renderPendientes() {
    ...
    const pendientes = activos
      .filter(p => !evalSet.has(`${p.legajo}|${p.empresa}`) && !esReingreso(p))
          ...
          <div class="pind__grupo-header-izq">
            ...
            <div class="pind__grupo-badges">
              ...
              ${previa
                ...
                : esReingreso(persona)
                  ? '<span class="pind__grupo-badge pind__grupo-badge--desvinc" style="color:#d97706;border-color:#d97706">Reingreso detectado — no debería necesitar período de prueba</span>'
                  : `<span class="pind__grupo-badge pind__grupo-badge--desvinc" style="color:${plazo.color};border-color:${plazo.color}">${plazo.texto}</span>`}
            </div>
          </div>
        </div>
        ${!previa && esReingreso(persona) ? `
        <p class="pres__vacio-small" style="margin-top:var(--espacio-s);color:#d97706">
          Esta fecha de ingreso es más nueva que el registro que tenemos de esta persona — probablemente renunció
          y fue recontratada (ej. cobro de FCL) sin dejar de trabajar en la práctica. Ya tiene antigüedad real, así
          que en principio no le corresponde período de prueba (le va a tocar la evaluación anual cuando corresponda).
          Completá este formulario solo si estás seguro de que sí es un ingreso nuevo.
        </p>` : ''}

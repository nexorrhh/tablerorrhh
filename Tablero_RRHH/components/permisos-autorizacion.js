...
export async function renderizarPermisosAutorizacion(contenedor, alActualizarBadge) {
  ...
  function abrirModalAprobar(permisoId) {
    ...
    // Quién aprueba sale de la sesión activa (ya identificada por el login), no se vuelve
    // a pedir a mano — evita el paso redundante y que alguien tipee un nombre distinto al
    // que realmente está logueado.
    const quien = obtenerUsuario()?.nombre || '';
    const { modal, cerrar } = abrirModal('perm-modal-aprobar', `
      <div class="sol__modal" role="dialog" aria-modal="true">
        ...
        <p class="sol__modal-desc">Aprueba: <strong>${e(quien) || '—'}</strong></p>
    ...
    modal.querySelector('#perm-ap-ok').addEventListener('click', async () => {
      if (!quien) { alert('No se pudo identificar al usuario logueado. Volvé a iniciar sesión.'); return; }
  ...
  function abrirModalRechazar(permisoId) {
    ...
    const { modal, cerrar } = abrirModal('perm-modal-rechazar', `
      <div class="sol__modal" role="dialog" aria-modal="true">
        ...
        <p class="sol__modal-desc">${e(nombreEmpleado(p))} — ${TIPO_LABEL[p?.tipo] || e(p?.tipo)} · ${fmtFechaHora(p || {})}</p>
        <p class="sol__modal-desc">Rechaza: <strong>${e(quien) || '—'}</strong></p>
    ...
    setTimeout(() => textObs.focus({ preventScroll: true }), 40);

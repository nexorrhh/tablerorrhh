...
export async function renderizarPanelEvaluador(contenedor, evaluador) {
  ...
  let empleados = [], mapaClasif = new Map(), asignaciones = [];
  try {
    const [rE, mc, asig] = await Promise.all([
  ...
  function estadoDe(p) {
    ...
    const corrAnual = (tipo === 'mensual' || tipo === 'quincenal') && corresponde(p, anio, mapaClasif);
    const yaAnual = evaluadosAnualSet.has(clave);
    const dias = diasDesdeIngreso(p, hoy);
    const corrPrueba = !esReingreso(p) && dias >= 0 && dias <= VENTANA_VISIBLE_DIAS;
